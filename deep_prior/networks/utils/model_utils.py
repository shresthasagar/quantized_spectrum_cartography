...
from networks.utils.errors import InvalidArchitectureError
...
def create_discriminator(z_dim):
    """
    :param int z_dim: latent vector dimension
    :return: A PyTorch Sequential model that represents the discriminator of an AAE.
    :rtype: torch.nn.Sequential
    """
    return nn.Sequential(
        nn.Linear(in_features=z_dim,    out_features=z_dim//2),
        nn.ReLU(True),
        nn.Linear(in_features=z_dim//2, out_features=z_dim//4),
        nn.ReLU(True),
        nn.Linear(in_features=z_dim//4, out_features=z_dim//8),
        nn.ReLU(True),
        nn.Linear(in_features=z_dim//8, out_features=z_dim//16),
        nn.ReLU(True),
        nn.Linear(in_features=z_dim//16, out_features=1),
        nn.Sigmoid()
        )
