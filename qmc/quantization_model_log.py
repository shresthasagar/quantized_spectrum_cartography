...
from utils import LOG_OFFSET_7_ADJUSTED as LOG_OFFSET

def quantize(X, noise_std, bin_boundaries, offset=LOG_OFFSET):
    """
    ...
    """
    noise_X = torch.log(X+offset) + torch.randn(X.shape)*noise_std
...
def get_quantized_obs_from_ordinal(Y, bin_boundaries, noise_std):
    bin_boundaries = bin_boundaries.clone()
    # bin_boundaries[0] = -100000
    # bin_boundaries[-1] = 100000
...
def NMSE_LOG(T: torch.Tensor, T_target: torch.Tensor, offset:float):
    """
    Normalized mean squared error after taking log
    """
    T_log = torch.log(T + offset)
    T_target_log = torch.log(T_target + offset)
